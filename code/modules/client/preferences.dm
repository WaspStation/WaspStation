	...
	switch(href_list["task"])
		...
		if("input")
			switch(href_list["preference"])
				...
				if("squid_face")
					var/new_face
					new_face = input(user, "Choose your character's face:", "Character Preference") as null|anything in GLOB.squid_face_list
					if(new_face)
						features["squid_face"] = new_face
